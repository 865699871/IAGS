...
def buildSimulatedMultiGMP_CRBs(adjacencises, save_final_species_adjacencies, change_adjacency_number, divergence_level, current_level):
    ...
    if current_level == divergence_level:
        ...
        save_final_species_adjacencies.append(new_sp2)
        buildSimulatedMultiGMP_CRBs(new_sp2, save_final_species_adjacencies, change_adjacency_number,
...
def simulateMultiGMP_CRBs(outdir, change_adjacency_number):
    ...
    # duplication first
    ancestor_adjacency_dup = []
    for i in ancestor_adjacency:
        if i[0] == '$':
            newendpoint1 = '$'
            newendpoint2 = '$'
        else:
        ...
        ancestor_adjacency_dup.append([newendpoint1, newendpoint3])
        ancestor_adjacency_dup.append([newendpoint2, newendpoint4])

    random.shuffle(ancestor_adjacency_dup)
    dup_ancestor = copy.deepcopy(ancestor_adjacency_dup)
    dup_ancestor_change = copy.deepcopy(dup_ancestor[:change_adjacency_number * 2])
    dup_ancestor_change = changeAdj(dup_ancestor_change)
    dup_ancestor_change_unchange = copy.deepcopy(dup_ancestor[change_adjacency_number * 2:])
    new_dup_ancestor = dup_ancestor_change + dup_ancestor_change_unchange

    # change some adjacencies in each divergence
    save_final_species_adjacencies = [new_dup_ancestor]
    buildSimulatedMultiGMP_CRBs(new_dup_ancestor, save_final_species_adjacencies, change_adjacency_number * 2, divergence_level, current_level=0)
